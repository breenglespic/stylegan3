    ...
    def __call__(self, show=True):
        ...
        if show:
            ...
            with imgui_utils.item_width(viz.font_size * 8):
                _changed, (self.xlate.x, self.xlate.y) = imgui.input_float2(
                    "##xlate", self.xlate.x, self.xlate.y, format="%.4f"
                )
            ...
            with imgui_utils.item_width(-1 - viz.button_w - viz.spacing), imgui_utils.grayed_out(not self.xlate.anim):
                changed, speed = imgui.slider_float(
                    "##xlate_speed", self.xlate.speed, 0, 0.5, format="Speed %.5f", power=5
                )
                if changed:
        ...
        if show:
            ...
            with imgui_utils.item_width(-1 - viz.button_w - viz.spacing), imgui_utils.grayed_out(not self.rotate.anim):
                changed, speed = imgui.slider_float(
                    "##rotate_speed", self.rotate.speed, -1, 1, format="Speed %.4f", power=3
                )
                if changed:
