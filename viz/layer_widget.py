...
class LayerWidget:
    ...
    def __call__(self, show=True):
        ...
        if show:
            ...
            imgui.begin_child(
                "##list", width=width, height=height, border=True, flags=imgui.WINDOW_ALWAYS_VERTICAL_SCROLLBAR
            )
            ...
            with imgui_utils.item_width(-1 - viz.button_w - viz.spacing):
                _changed, self.img_scale_db = imgui.slider_float(
                    "##scale", self.img_scale_db, min_value=-40, max_value=40, format="Scale %+.1f dB"
                )
            imgui.same_line()
            ...
            with imgui_utils.grayed_out(base_channel_max == 0):
                with imgui_utils.item_width(-1 - viz.button_w - narrow_w * 2 - viz.spacing * 3):
                    _changed, self.base_channel = imgui.drag_int(
                        "##channel",
                        self.base_channel,
                        change_speed=0.05,
                        min_value=0,
                        max_value=base_channel_max,
            ...
            if imgui_utils.button(
                "Reset##channel", width=-1, enabled=(self.base_channel != 0 and base_channel_max > 0)
            ):
                self.base_channel = 0
            ...
            with imgui_utils.grayed_out(not self.fft_show):
                with imgui_utils.item_width(-1 - viz.button_w - viz.spacing):
                    _changed, self.fft_range_db = imgui.slider_float(
                        "##fft_range_db", self.fft_range_db, min_value=0.1, max_value=100, format="Range +-%.1f dB"
                    )
                imgui.same_line()
            ...
            with imgui_utils.grayed_out(not self.fft_show):
                with imgui_utils.item_width(-1 - viz.button_w - viz.spacing):
                    _changed, self.fft_beta = imgui.slider_float(
                        "##fft_beta", self.fft_beta, min_value=0, max_value=50, format="Kaiser beta %.2f", power=2.63
                    )
                imgui.same_line()
        ...
        viz.args.update(
            sel_channels=self.sel_channels,
            base_channel=self.base_channel,
            img_scale_db=self.img_scale_db,
