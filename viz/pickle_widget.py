...
class PickleWidget:
    ...
    def __call__(self, show=True):
        ...
        if show:
            ...
            changed, self.user_pkl = imgui_utils.input_text(
                "##pkl",
                self.user_pkl,
                1024,
            ...
            if os.path.isdir(parent):
                for entry in os.scandir(parent):
                    if entry.is_dir() and run_regex.fullmatch(entry.name):
                        items.append(
                            dnnlib.EasyDict(type="run", name=entry.name, path=os.path.join(parent, entry.name))
                        )
                    if entry.is_file() and pkl_regex.fullmatch(entry.name):
                        items.append(
                            dnnlib.EasyDict(type="pkl", name=entry.name, path=os.path.join(parent, entry.name))
                        )
