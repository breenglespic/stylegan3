...
def save_image_grid(img, fname, drange, grid_size, run=None, caption="", step=0):
    lo, hi = drange
    ...
    assert C in [1, 3]
    if C == 1:
        img_ = PIL.Image.fromarray(img[:, :, 0], "L")
    if C == 3:
        img_ = PIL.Image.fromarray(img, "RGB")

    img_.save(fname)
    if run is not None:
        run.track(aim.Image(img_, caption=caption, quality=50, optimize=True), name="images", step=step)
    ...
    while True:
        ...
        if (rank == 0) and (image_snapshot_ticks is not None) and (done or cur_tick % image_snapshot_ticks == 0):
            ...
            save_image_grid(
                ...
                grid_size=grid_size,
                run=run,
                caption=f"fakes{cur_nimg//1000:06d}.png",
                step=cur_nimg,
            )
            ...
            for metric in metrics:
                ...
                if rank == 0:
                    metric_main.report_metric(
                        result_dict, run_dir=run_dir, snapshot_pkl=snapshot_pkl, run=run, step=cur_nimg
                    )
